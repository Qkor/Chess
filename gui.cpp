...
void Gui::set_piece_texture(Sprite &piece, char type){
    int piece_type;
...
void Gui::set_position(){
    for(int i=0;i<8;i++)
        for(int j=0;j<8;j++)
            set_piece_texture(pieces[i][j], board[i][j]);
}

void Gui::refresh_position(){
    board = game.get_position();
    set_position();
}

void Gui::display_board(){
...
void Gui::move(char promoted_piece){
    mov="";
